...
import {getWali} from '../../WaliService'; // Fetch all Penggunas (users) to select as Wali
...
const EditLansiaForm = () => {
  ...
  useEffect(() => {
    const fetchDropdownOptions = async () => {
      try {
        ...
        const waliRes = await getWali(); // Fetch all users to select as Wali

        setPekerjaanOptions(pekerjaanRes.data);
        setPendidikanOptions(pendidikanRes.data);
        setWaliOptions(waliRes.data);
      } catch (error) {
    ...
    const fetchLansiaDetails = async () => {
      try {
        ...
        axios
          ...
          if (lansia.provinsi_ktp_lansia) {
            const regencyResponse = await
            axios.get(`${import.meta.env.VITE_API_URL}/api/location/provinsi/${lansia.provinsi_ktp_lansia}/regencies`);
            setRegenciesLansia(regencyResponse.data);

            if (lansia.kota_ktp_lansia) {
              const districtResponse = await
              axios.get(`${import.meta.env.VITE_API_URL}/api/location/regencies/${lansia.kota_ktp_lansia}/districts`);
              setDistrictsLansia(districtResponse.data);

              if (lansia.kecamatan_ktp_lansia) {
                const villageResponse = await
                axios.get(`${import.meta.env.VITE_API_URL}/api/location/districts/${lansia.kecamatan_ktp_lansia}/villages`);
                setVillagesLansia(villageResponse.data);
              }
            }
          }

          if (lansia.provinsi_domisili_lansia) {
            const regencyDomResponse = await
            axios.get(`${import.meta.env.VITE_API_URL}/api/location/provinsi/${lansia.provinsi_domisili_lansia}/regencies`);
            setRegenciesLansiaDom(regencyDomResponse.data);

            if (lansia.kota_domisili_lansia) {
              const districtDomResponse = await
              axios.get(`${import.meta.env.VITE_API_URL}/api/location/regencies/${lansia.kota_domisili_lansia}/districts`);
              setDistrictsLansiaDom(districtDomResponse.data);

              if (lansia.kecamatan_domisili_lansia) {
                const villageDomResponse = await
                axios.get(`${import.meta.env.VITE_API_URL}/api/location/districts/${lansia.kecamatan_domisili_lansia}/villages`);
  ...
  const validateForm = () => {
    ...
    if (!formData.no_kk_lansia) 
      newErrors.no_kk_lansia = 'Nomor KK wajib diisi';
    if (!formData.nik_lansia) 
      newErrors.nik_lansia = 'NIK wajib diisi';
    ...
    if (!formData.nama_lansia) 
      newErrors.nama_lansia = 'Nama Lansia wajib diisi';
    if (!formData.tempat_lahir_lansia) 
      newErrors.tempat_lahir_lansia = 'Tempat Lahir wajib diisi';
    if (!formData.tanggal_lahir_lansia) 
      newErrors.tanggal_lahir_lansia = 'Tanggal Lahir wajib diisi';
    if (!formData.jenis_kelamin_lansia) 
      newErrors.jenis_kelamin_lansia = 'Jenis Kelamin wajib diisi';
    if (!formData.alamat_ktp_lansia) 
      newErrors.alamat_ktp_lansia = 'Alamat KTP wajib diisi';
    if (!formData.alamat_domisili_lansia) 
      newErrors.alamat_domisili_lansia = 'Alamat Domisili wajib diisi';
    if (formData.email_lansia && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email_lansia)) {
      newErrors.email_lansia = 'Email tidak valid';
    }

    if (!formData.wali) newErrors.wali = 'Wali wajib diisi'; // Ensure Wali is selected
                  ...
                  >
                    ...
                    {waliOptions.map((wali) => (
                      <option key={wali.id} value={wali.id}>
                        {wali.nama_wali} ({wali.no_hp_wali})
                      </option>
                    ))}
                  </select>
