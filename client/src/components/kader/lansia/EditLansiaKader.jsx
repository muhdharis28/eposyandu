...
import {getPengguna} from '../../PenggunaService'; // Fetch all Penggunas (users) to select as Wali
...
const EditLansiaForm = () => {
  ...
  useEffect(() => {
    const fetchDropdownOptions = async () => {
      try {
        ...
        const waliRes = await getPengguna(); // Fetch all users to select as Wali
  ...
  const validateForm = () => {
    ...
    if (!formData.wali) newErrors.wali = 'Wali is required'; // Ensure Wali is selected
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateLansia(id, formData); // Pass the form data, including Wali
      ...
      <div className="flex flex-grow transition-all duration-500 ease-in-out">
        ...
        <div className="flex-1 bg-gray-100 p-6 transition-all duration-500 ease-in-out mt-16">
          ...
          <nav className="text-gray-600 mb-4">
            <Link to="/kader-lansia" className="hover:underline">Lansia List</Link> &gt; Edit Lansia
          </nav>

          <div className="bg-white p-6 rounded-lg shadow-lg">
            <h1 className="text-2xl font-bold mb-4">Edit Lansia</h1>
            <form onSubmit={handleSubmit}>
              ...
              >
                {isSubmitting ? 'Updating...' : 'Update Lansia'}
              </button>
            </form>
          </div>
        </div>
