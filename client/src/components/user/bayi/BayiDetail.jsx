...
const BayiDetail = () => {
  ...
  const barChartOptions = {
    ...
    plugins: {
      legend: { position: 'top' },
      title: { display: true, text: 'Grafik Perkembangan Bayi' }
    }
  };
                ...
                <div className="mt-6">
                  <div className="border-b border-gray-200 mb-4">
                    <nav className="flex space-x-4">
                      <button onClick={() => setActiveTab('perkembangan')}
                        className={`py-2 px-4 text-sm font-medium ${activeTab === 'perkembangan' ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-600'}`}>
                        Perkembangan Bayi
                      </button>
                        ...
                        <div className="space-y-4">
                          {perkembanganList.map((perkembangan) => (
                            <div key={perkembangan.id}
                              ...
                              <p className="text-gray-600"><strong>Kader: </strong>{perkembangan.kaderDetail?.nama ?? '-'}</p>
                              <p className="text-gray-600"><strong>Hasil Perkembangan: </strong>{perkembangan.keterangan}</p>
                            </div>
                          ))}
                        </div>
                      ) : (
      ...
      {selectedPerkembangan && (
        <Modal isOpen={modalIsOpen} onRequestClose={closeModal} style={customModalStyles} contentLabel="Perkembangan Details">
          <div className="relative bg-white p-6 ">
            <button className="absolute top-2 right-2 text-gray-500 hover:text-gray-700" onClick={closeModal}><FaTimes /></button>
            <h2 className="text-2xl font-bold text-blue-700 mb-4">Detail Perkembangan</h2>
            ...
            <p><strong>Lingkar Kepala:</strong> {selectedPerkembangan.lingkar_kepala} cm</p>
