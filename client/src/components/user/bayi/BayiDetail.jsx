...
const BayiDetail = () => {
  const {id} = useParams();
  const [balita,
    setBalita] = useState(null);
  const [isLoading,
    setIsLoading] = useState(true);
  const [perkembanganList,
    setPerkembanganList] = useState([]);
  const [modalIsOpen,
    setModalIsOpen] = useState(false);
  const [selectedPerkembangan,
    setSelectedPerkembangan] = useState(null);
  const {isSidebarCollapsed, toggleSidebar} = useSidebar();
  const [activeTab,
    setActiveTab] = useState('perkembangan');
  ...
  const barChartData = {
    labels: perkembanganList.map((perkembangan) => new Date(perkembangan.tanggal_kunjungan).toLocaleDateString()),
    datasets: [
      {
        label: 'Berat Badan',
    ...
    <div className="h-screen flex flex-col">
      ...
      <div className="flex flex-grow transition-all duration-500 ease-in-out">
        ...
        <div
          ...
          <nav className="text-gray-600 mb-4">
            <Link to="/user-balita" className="hover:underline">Bayi List</Link>
            &gt; Detail Bayi
          </nav>
                    ...
                    <div className="border-b border-gray-200 mb-4">
                      <nav className="flex space-x-4">
                        <button
                          ...
                          className={`py-2 px-4 text-sm font-medium ${activeTab === 'perkembangan'
      ...
      {selectedPerkembangan && (
        <Modal
          ...
          contentLabel="Perkembangan Details">
          <div className="relative bg-white p-6 ">
            ...
            <p>
              <strong>Tanggal Perkembangan:</strong>
              {new Date(selectedPerkembangan.tanggal_kunjungan).toLocaleDateString()}
            </p>
            <p>
              <strong>Status Gizi:</strong>
              {selectedPerkembangan.status_gizi || 'Tidak ada data'}
            </p>
            <p>
              <strong>Berat Badan:</strong>
              {selectedPerkembangan.berat_badan || 'Tidak ada catatan'}
            </p>
            <p>
              <strong>Tinggi Badan:</strong>
              {selectedPerkembangan.tinggi_badan || 'Tidak ada catatan'}
            </p>
            <p>
              <strong>Lingkar Kepala:</strong>
              {selectedPerkembangan.lingkar_kepala || 'Tidak ada catatan'}
            </p>
            <p>
              <strong>Riwayat Penyakit:</strong>
              {selectedPerkembangan.riwayat_penyakit || 'Tidak ada catatan'}
            </p>
            <p>
              <strong>Tipe Imunisasi:</strong>
              {selectedPerkembangan.tipe_imunisasi || 'Tidak ada catatan'}
            </p>
            <p>
              <strong>Tipe Vitamin:</strong>
              {selectedPerkembangan.tipe_vitamin || 'Tidak ada catatan'}
            </p>
          </div>
        </Modal>
      )}
